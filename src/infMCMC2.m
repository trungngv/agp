function [post nlZ dnlZ] = infMCMC2(hyp, mean, cov, lik, x, y, par)
...
%
% Modified by Trung Nguyen to return the samples.
%
...
[alpha,Na,F] = sample(K,cK,m,y,lik,hyp.lik, N,Nb,Ns, alg);  % sample w/o annealing
...
function [alpha,Na,F] = sample(K,cK,m,y,lik,hyp, N,Nb,Ns, alg, varargin)
  if strcmpi(alg,'hmc')
    [alpha,Na,F] = sample_hmc(K, m,y,lik,hyp, N,Nb,Ns, varargin{:});
  else
    [alpha,Na,F] = sample_ess(cK,m,y,lik,hyp, N,Nb,Ns, varargin{:});
  end

%% sample using elliptical slices
function [alpha,T,F] = sample_ess(cK,m,y,lik,hyp, N,Nb,Ns, taus)
  ...
  for t=1:T
    ...
    if mod(t,Ns)==0                                   % keep one state out of Ns
      ...
      if t/Ns>Nb && mod(t/Ns-Nb,100)==0, fprintf('.'); end
    end
  end
  alpha = cK\(cK'\F);
...
function [alpha,Na,F] = sample_hmc(K,m,y,lik,hyp, N,Nb,Ns, taus)
